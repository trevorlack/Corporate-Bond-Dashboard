...
os.chdir("S:\Trevor\MySQL Data\IG Index\Loader")
FileList=[]
Badlist=[]
...
for file in list(glob.glob("*.csv")):
    ...
    ws = wb.Worksheets(tabname)

    ws.Range("E5").Select()
    Cell = x1.Sheets(tabname).Range("E5").Value
    ...
    if Cell == "REPO":
        ...
        cc = x1.Sheets(tabname).Range("A1").Value
        c=int(cc)
        r=1
        for r in range(1,c):
            #Search for empty rows of data and delete them
            CUSIP=x1.Sheets(tabname).Range("C%d" % r).Value
            if CUSIP == None:
                ws.Rows(r).Delete()
    else:
        Badlist.append(file)
